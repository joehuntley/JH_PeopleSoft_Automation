Attribute VB_Name = "QueueProcessors"
Option Explicit


Private Const Q_MAX_CONSECUTIVE_FAILURES = 5
...
End Sub
Public Sub Process_PO_FromQuote_Q()
    ...
    Set queueTableRange = ThisWorkbook.Worksheets("PO_FromQuote_Q").Range("4:65536")
    Const HEADER_ROWS_SIZE = 0
    ...
    For curRow = 1 + HEADER_ROWS_SIZE To ssMap.ROW_COUNT + HEADER_ROWS_SIZE
        ...
        If idxParent > 0 Then
            ...
            If idxChild = 1 Then
                PO_CFQs(idxParent).E_QUOTE_NBR = queueTableRange.Cells(curRow, C_E_QUOTE_NBR).Value
...
Public Sub Process_PO_Receipt_Queue()
    ...
    For idxParent = 1 To ssMap.PARENT_COUNT
        If Rcpts_DoNotProcess(idxParent) = False Then
            ...
            result = PeopleSoft_PurchaseOrder_ProcessReceipt(session, Rcpts(idxParent))
            ...
            If result Then ' Receipt created OK
                ...
                ElseIf Rcpts(idxParent).ReceiveMode = RECEIVE_ALL Then
                    ...
                    Dim rcptItemLineStr As String, rcptItemQtyStr As String, rcptItemIDStr As String
                    Dim recpTransItemDescStr As String
                    
                    curRow = ssMap.PARENT_MAP(idxParent).MAP_CHILD_TO_ROW(1)
                    rcptItemLineStr = ""
                    rcptItemQtyStr = ""
                    rcptItemIDStr = ""
                    recpTransItemDescStr = ""
                    ...
                    For i = 1 To Rcpts(idxParent).ReceiptItemCount
                        ...
                        rcptItemIDStr = rcptItemIDStr & Rcpts(idxParent).ReceiptItems(i).ITEM_ID & " (" & Rcpts(idxParent).ReceiptItems(i).CATS_FLAG & ")" & vbCrLf
                        recpTransItemDescStr = recpTransItemDescStr & Rcpts(idxParent).ReceiptItems(i).TRANS_ITEM_DESC & vbCrLf
                    Next i
            ...
            Else
                ...
                If Rcpts(idxParent).ReceiveMode = RECEIVE_ALL Then childCount = 11
            ...
            ' update 2.10.1: ignore individual receive errors if receive_all
            If Rcpts(idxParent).ReceiveMode = RECEIVE_SPECIFIED Then
                For idxChild = 1 To childCount
                    curRow = ssMap.PARENT_MAP(idxParent).MAP_CHILD_TO_ROW(idxChild)
        ...
        If idxParent > 0 Then
           ...
           If queueTableRange.Cells(curRow, C_PO_FLG_SEND_TO_VENDOR).Value <> "" Then
                Select Case UCase(queueTableRange.Cells(curRow, C_PO_FLG_SEND_TO_VENDOR).Value)
                    Case "X", "Y", "YES":
                        PO_COs(idxParent).PO_HDR_FLG_SEND_TO_VENDOR = SetAsChecked
                    Case "N", "NO":
                        PO_COs(idxParent).PO_HDR_FLG_SEND_TO_VENDOR = SetAsUnchecked
                    Case Else
                        PO_COs(idxParent).PO_HDR_FLG_SEND_TO_VENDOR = KeepExistingValue
                End Select
           End If
